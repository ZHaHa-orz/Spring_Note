...
public class JavaConfiguration {
     ...
     * 问题4: 如何引用其他的ioc组件
     *      直接调用对方的bean方法即可
     *      直按形参变量进行引入，要求必须有对应的组件，如果有多个，形参名=组件id标识即可
     */
    ...
    @Bean(name="zhh", initMethod = "init", destroyMethod = "close")
    ...
    @Bean
    public JdbcTemplate jdbcTemplate1(DataSource dataSource, DataSource zhh){
        JdbcTemplate jdbcTemplate = new JdbcTemplate();
        // 需要DataSource 需要ioc容器的其他组件
        // 方案2: 形参列表声明想要的组件类型，可以是一个也可以是多个! ioc容器也会注入
        // 如果没有bean: 形参变量注入。要求必须有对应的类型的组件，如果没有抛异常!
        // 如果有多个bean: 可以使用形参名称等同于对应的bean id标识
        jdbcTemplate.setDataSource(zhh);
        return jdbcTemplate;
    }
}
